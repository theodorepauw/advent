...
pub fn solve() -> crate::util::Result<()> {
    ...
    let order = paragraphs
        .next()
        .expect("no  numbers to draw")
        .split(',')
        .map(str_to_int);

    let mut boards: Vec<(Vec<(usize, bool)>, bool)> = paragraphs
        .map(|s| {
            s.lines()
                .flat_map(|s| s.split_whitespace().map(|s| (str_to_int(s), false)))
                .collect()
        })
        .map(|v| (v, false))
        .collect();
            ...
            for i in 0..b.len() {
                ...
                if n == &x {
                    *marked = true;
                    ...
                    if (row_start..row_start + 5).all(|j| b[j].1)
                        || (1..5).all(|j| b[(i + j * 5) % 25].1)
                    {
                        *bingo = true;
                        p2 = x * b
                            .iter()
