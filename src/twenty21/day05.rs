...
use std::{io::{self, Write}, collections::HashMap, str::SplitN};
...
pub fn solve() -> crate::util::Result<()> {
    ...
    let parse_point = |line: &mut SplitN<&str>| { 
        let v: Vec<i32> = line.next().expect("invalid line").splitn(2, ',').map(|s| s.parse::<i32>().expect("s to usize err")).collect(); 
        (v[0], v[1])
    };
    let min_max = |a, b| if a>b { (b, a) } else { (a, b) };
    let add_line = |map: &mut HashMap<Point, usize>, p: Point| *map.entry(p).or_insert(0) += 1;
    let count_overlaps = |map: HashMap<Point, usize>| map.values().filter(|&&overlaps| overlaps>1).count();
    ...
    writeln!(io::stdout(), "Day 05 Part 1: {}\nDay 05 Part 2: {}", count_overlaps(p1), count_overlaps(p2))?;
    Ok(())
}